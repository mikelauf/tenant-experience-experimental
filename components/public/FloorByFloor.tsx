...
/** Overview, each public venue from the ground up, then the crown */
const stopsFor = ({ venues, copy }: Tenant): Stop[] => [
  { level: null, readout: "The building", ...copy.public.floorIntro },
  ...[...venues]
...
/** Sticky 3D tower; the marker rides up it as each stop scrolls into view. */
export function FloorByFloor() {
  const t = useTenant();
  const { building } = t;
  const stops = stopsFor(t);
  const ground = t.levels.find((l) => l.n === 0)?.label ?? "Street";
  const poster = t.copy.public.towerPoster;
        ...
        <div className="sticky top-[var(--nav-h)] z-0 h-[52svh] p-2 lg:col-span-7 lg:h-[calc(100svh-var(--nav-h))] lg:px-0 lg:py-4">
          <div className="relative h-full overflow-hidden rounded-[26px]">
            ...
            <Tower
              ...
              poster={<Image src={poster.src} alt="" fill sizes="60vw" className="object-cover opacity-80" style={{ objectPosition: poster.pos }} />}
            />
            <p
              className={cn(
            ...
            <div className="pointer-events-none absolute left-4 top-4 flex items-end gap-4 lg:left-6 lg:top-6">
              <div className="flex h-[76px] min-w-[92px] items-center justify-center rounded-2xl bg-black/40 px-4 backdrop-blur-md lg:h-[96px] lg:min-w-[116px]">
                <span className="t-num text-[3rem] font-medium leading-none text-accent-glow lg:text-[3.75rem]">
          ...
          {stops.map((s, i) => (
            ...
            >
              <div className={cn("transition-opacity duration-700", idx === i ? "opacity-100" : "opacity-35")}>
                <p className="t-meta tabular">{s.level == null ? `${building.address}` : s.level === 0 ? `${ground} level` : `Level ${s.level}`}</p>
